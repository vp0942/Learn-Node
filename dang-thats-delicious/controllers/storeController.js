...
exports.getStores = async (req, res) => {
  ...
  const page = req.params.page || 1; // default to page 1 if no page is specified in the URL
  const limit = 4; // limit the number of stores per page
  const skip = (page * limit) - limit; // calculate the number of stores to skip
  const storesPromise = Store
    .find() // find stores
    .skip(skip) // skip the number of stores
    .limit(limit) // limit the number of stores
    .sort({ created: 'desc' }); // sort the stores by the date they were created starting from the most recent

  const countPromise = Store.count(); // count the total number of stores
  const [stores, count] = await Promise.all([storesPromise, countPromise]);// wait for both promises to resolve
  const pages = Math.ceil(count / limit); // calculate the number of pages
  if (!stores.length && skip) {
    req.flash('info', `Hey! You asked for page ${page}. But that doesn't exist. So I put you on page ${pages}`);
    res.redirect(`/stores/page/${pages}`);
    return;
  } // redirect the user to the last page if the requested page does not exist
  // console.log(stores);
